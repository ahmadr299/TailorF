 ...
 * Use the {@link Tailor_Due_Orders_Fragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class Tailor_Due_Orders_Fragment extends Fragment {
    private RecyclerView recyclerView_tailorComplete_order;
    private TextView tv_tailotComplete_order;
    ...
    public Tailor_Due_Orders_Fragment() {
     ...
     * @return A new instance of fragment Tailor_Due_Orders_Fragment.
     */
    // TODO: Rename and change types and number of parameters
    public static Tailor_Due_Orders_Fragment newInstance(String param1, String param2) {
        Tailor_Due_Orders_Fragment fragment = new Tailor_Due_Orders_Fragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view=inflater.inflate(R.layout.fragment_tailor__due__orders_, container, false);
        tv_tailotComplete_order = view.findViewById(R.id.tv_tailotComplete_order);
        recyclerView_tailorComplete_order = view.findViewById(R.id.recyclerView_tailorComplete_order);
                        ...
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            if(task.isSuccessful())
                            {
                                ...
                                if (task.getResult().size()>0)
                                {
                                    ...
                                    if (Orders>0)
                                    {
                                        Tailor_Active_OrdersAdapter adapter = new Tailor_Active_OrdersAdapter(list, view.getContext(),"Tailor Complete Orders");
                                        recyclerView_tailorComplete_order.setLayoutManager(new GridLayoutManager(view.getContext(), 1));
                                        recyclerView_tailorComplete_order.setAdapter(adapter);
                                        adapter.notifyDataSetChanged();
                                    }
                                    else
                                    {
                                        tv_tailotComplete_order.setVisibility(View.VISIBLE);
                                        recyclerView_tailorComplete_order.setVisibility(View.GONE);
                                        tv_tailotComplete_order.setText("No New Order Yet");
                                    }
                                }
                                else
                                {
